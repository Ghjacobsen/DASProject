...
TIME_BIN_SIZE = 640          # Target time dimension after binning (no temporal cropping)
MEDIAN_FILTER_ENABLED = True  # Apply 2D median filtering before tiling
MEDIAN_FILTER_SIZE = 3        # Kernel size for median filter (square)
    ...
    def preprocess(self, output_folder: str | Path) -> None:
        ...
        while file_idx + files_per_window <= len(sorted_files):
            try:
                ...
                # 5.1. Median filtering to suppress salt-and-pepper noise while preserving edges
                if MEDIAN_FILTER_ENABLED:
                    data = median_filter(data, size=MEDIAN_FILTER_SIZE)
                    print(f"  Applied 2D median filter (size {MEDIAN_FILTER_SIZE})")
                ...
                if IMAGE_MODE == 'VISUALIZATION':
                    ...
                    # Bin time dimension to TIME_BIN_SIZE (no temporal cropping)
                    if n_time != TIME_BIN_SIZE:
                        zoom_factor = TIME_BIN_SIZE / n_time
                        data = zoom(data, (zoom_factor, 1.0), order=1)
                        n_time = TIME_BIN_SIZE
                        print(f"  Binned time dimension: {n_time} → {TIME_BIN_SIZE} pixels")

                    # Force correct downsampling for space
                    ...
                    # Slide only spatially; time dimension is fixed at TIME_BIN_SIZE
                    for s_start in range(0, n_space, tile_size):
                        s_end = min(s_start + tile_size, n_space)

                        tile = data[:, s_start:s_end]

                        # Pad only spatial dimension if needed
                        if tile.shape[1] < tile_size:
                            pad_s = tile_size - tile.shape[1]
                            tile = np.pad(tile, ((0, 0), (0, pad_s)), mode='constant')
