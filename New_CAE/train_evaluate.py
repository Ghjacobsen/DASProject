...
from data_loader import load_train_validation_data, ensure_test_split, load_test_data
...
def run_hyperparameter_search():
    ...
    train_ds, val_ds = load_train_validation_data()
    ...
    for run_num, params in enumerate(combinations):
        ...
        model.load_weights('temp_best_model.h5') 
        ...
        if best_f1_local > best_f1:
            ...
            # Save champion model to root
            model.save('champion_model.h5')
    ...
    # Append best-per-combo rows to a single registry file
    try:
        registry = Path('runs') / 'registry.csv'
        # include run_id in each row
        results_df.to_csv(registry, mode='a', header=not registry.exists(), index=False)
...
def evaluate_on_test(best_params, threshold: float):
    """
    Evaluate the champion model on the held-out test set.
    """
    print("\n--- Evaluating Champion on Test Holdout ---")

    if os.path.exists('champion_model.h5'):
        best_model = tf.keras.models.load_model('champion_model.h5', compile=False)
    ...
    test_ds, test_filenames, test_labels = load_test_data()
    ...
    out_csv = 'champion_test_predictions.csv'
...
if __name__ == "__main__":
    ...
    missing = [c for c in expected_classes if not os.path.isdir(os.path.join(getattr(CFG, 'TRAIN_DIR', 'train'), c))]
    if missing:
        raise FileNotFoundError(f"Missing class folders in '{getattr(CFG, 'TRAIN_DIR', 'train')}': {missing}")
    # Create/ensure small held-out test split
    ensure_test_split(move_count_per_class=getattr(CFG, 'TEST_SAMPLES_PER_CLASS', 5), seed=getattr(CFG, 'RANDOM_SEED', 42))
    ...
    try:
        ...
        best_params, best_threshold = run_hyperparameter_search()
        
        # Step 2: Evaluate champion on the held-out test set
        evaluate_on_test(best_params, best_threshold)
